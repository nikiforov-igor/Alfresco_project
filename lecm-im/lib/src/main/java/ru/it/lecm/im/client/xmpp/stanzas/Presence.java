...
public class Presence extends AbstractStanza {
	...
	public enum Type {
		available, error, probe, subscribe, subscribed, unavailable, unsubscribe, unsubscribed,invisible
	}
	...
	public Presence(Type type, JID from, JID to, Show show, String status, Integer priority) {
		super("presence");
		setType(type);
		setFrom(from);
		setTo(to);
	...
	public void setShow(Show value) {
		String v = (value != null && (value != Show.notSpecified && value != Show.unknown)) ? value.toString() : null;
	...
	public void setType(Type type) 
	{
		...
		{
			setAttribute("type", (type == null||type.equals(Type.available)||type.equals(Type.invisible)) ? null : type.name());
		}
		else
		{
			setAttribute("type", (type == null||type.equals(Type.available)) ? null : type.name());
		}
		/*
		if(!Session.instance().getServerType().equals(ServerType.openfire))
		{
			if(type!=null&&(type.equals(Type.available)||type.equals(Type.invisible)))
			{
