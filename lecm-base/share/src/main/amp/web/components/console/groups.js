...
(function()
{
   ...
   Alfresco.ConsoleGroups = function(htmlId)
   {
      ...
      YAHOO.extend(SearchPanelHandler, Alfresco.ConsolePanelHandler,
      {
         ...
         {
            var url = Alfresco.constants.PROXY_URI + "api/groups/" + encodeURIComponent(shortName.replace(/%/g,'%25'));
         ...
         _removeGroup: function ConsoleGroups_SearchPanelHandler__removeGroup(fullName, parentShortName, displayName)
         {
            ...
            var url = Alfresco.constants.PROXY_URI + "api/groups/" + encodeURIComponent(parentShortName.replace(/%/g,'%25')) + "/children/" + encodeURIComponent(fullName);
            this._doDeleteCall(url, displayName);
         },
         ...
         _removeUser: function ConsoleGroups_SearchPanelHandler__removeUser(groupId, userId, userDisplayName)
         {
            ...
            {
               method: Alfresco.util.Ajax.DELETE,
               url: Alfresco.constants.PROXY_URI + "api/groups/" + encodeURIComponent(groupId.replace(/%/g,'%25')) + "/children/" + encodeURIComponent(userId),
               successCallback:
               {
                  fn: function(o)
                  {
         ...
         _addToGroup: function ConsoleGroups_SearchPanelHandler__addToGroup(objectId, parentGroupShortName, successMessage, failureMessage)
         {
            ...
            {
               url: Alfresco.constants.PROXY_URI + "api/groups/" + encodeURIComponent(parentGroupShortName.replace(/%/g,'%25')) + "/children/" + encodeURIComponent(objectId),
               successCallback:
               {
                  fn: function(o)
                  {
      ...
      YAHOO.extend(UpdatePanelHandler, Alfresco.ConsolePanelHandler,
      {
         ...
         onUpdate: function ConsoleGroups_UpdatePanelHandler_onUpdate()
         {
            ...
            {
               url: Alfresco.constants.PROXY_URI + "api/groups/" + encodeURIComponent(parent.group.replace(/%/g,'%25')),
               successCallback:
               {
                  fn: success,
                  scope: this
         ...
         updateGroupRequest: function ConsoleGroups_UpdatePanelHandler_updateGroupRequest(shortName, groupObj, successCallback)
         {
            ...
            {
               url: Alfresco.constants.PROXY_URI + "api/groups/" + encodeURIComponent(shortName.replace(/%/g,'%25')),
               dataObj: groupObj,
   ...
   YAHOO.extend(Alfresco.ConsoleGroups, Alfresco.ConsoleTool,
   {
      ...
      getParentGroups: function ConsoleGroups_getParentGroups(shortName, successCallback, failureMessage)
      {
         Alfresco.util.Ajax.jsonGet(
         {
            url:  Alfresco.constants.PROXY_URI + "api/groups/" + encodeURIComponent(shortName.replace(/%/g,'%25')) + "/parents?level=ALL&maxSize=10",
            successCallback:
            {
               fn: function(o)
               {
