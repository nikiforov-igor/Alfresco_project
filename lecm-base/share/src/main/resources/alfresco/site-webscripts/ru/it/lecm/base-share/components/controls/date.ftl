    ...
    <div class="viewmode-field">
        ...
        <span class="viewmode-label">${field.label?html}:</span>
         <span class="viewmode-value">
             ...
             <span id="${controlId}-date" class="date-entry"
...
<#elseif !multiValued>
    <#if jsDisabled>
        ...
        <input id="${fieldHtmlId}" name="${field.name}" type="text" class="date-entry" value="${field.value?html}"
    ...
    <#else>
        ...
        <script type="text/javascript">//<![CDATA[
        (function () {
            new LogicECM.DatePicker("${controlId}", "${fieldHtmlId}").setOptions(
                    {
        ...
        <label for="${controlId}-date">${field.label?html}:<#if field.mandatory><span
                class="mandatory-indicator">${msg("form.required.fields.marker")}</span></#if></label>
        <input id="${controlId}-date" name="-" type="text" class="date-entry"
               <#if field.description??>title="${field.description}"</#if> <#if disabled>disabled="true"
               <#else>tabindex="0"</#if> />

        <#if disabled == false>
            <a id="${controlId}-icon"><img src="${url.context}/res/components/form/images/calendar.png"
                                           class="datepicker-icon" tabindex="0"/></a>
        </#if>
