...
LogicECM.module.WCalendar.Absence = LogicECM.module.WCalendar.Shedule || {};


LogicECM.module.WCalendar.Shedule.reiterationRulesValidation = function Shedule_reiterationRulesValidation() {
	var valid = true;
	var errorMessage;
	...
	if (reiterationType.value === "week-days") {
		...
		if (daysChecked.length < 1) {
			valid = false;
			errorMessage = Alfresco.component.Base.prototype.msg("message.error.shedule.reiteration-rules-validation.week-days");
		}
		...
		if (monthDays.value.length < 1) {
			valid = false;
			errorMessage = Alfresco.component.Base.prototype.msg("message.error.shedule.reiteration-rules-validation.month-days");
		}
		...
		if (isNaN(workingDaysNum) || isNaN(nonWorkingDaysNum) || workingDaysNum < 1 || nonWorkingDaysNum < 1) {
			valid = false;
			errorMessage = Alfresco.component.Base.prototype.msg("message.error.shedule.reiteration-rules-validation.shift-work");
		}
	} else {
		valid = false;
		errorMessage = Alfresco.component.Base.prototype.msg("message.error.shedule.reiteration-rules-validation.reiteration-type");
	}
