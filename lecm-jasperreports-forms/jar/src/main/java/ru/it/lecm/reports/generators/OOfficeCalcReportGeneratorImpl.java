 ...
 * @author snovikov
 */
public class OOfficeCalcReportGeneratorImpl extends ReportGeneratorBase {
	private static final transient Logger logger = LoggerFactory.getLogger(OOfficeCalcReportGeneratorImpl.class);

	private final static Object _lockerUniqueGen = new Object();

	/**
	 * файловое расширение для openOffice calc файла
	 */
	private static final String OOCalc_FILEEXT = ".ods";

	/**
	 * файловое расширение для временного openOffice calc файла
	 */
	private static final String TEMP_OOCalc_FILEEXT = "_tmp" + OOCalc_FILEEXT;

	/**
	 * файловая маска-суффикс для временного openOffice calc файла генерируемого Шаблона Отчёта
	 */
	private static final String FMT_SFX_TEMP_RESULT_FILENAME_S = "-report_ready_%s" + TEMP_OOCalc_FILEEXT;

	/**
	 * файловая маска-суффикс для обычного генерируемого openOffice-отчёта
	 */
	private static final String FMT_SFX_RESULT_FILENAME_S = "-report_ready_%s" + OOCalc_FILEEXT;
	 ...
	 */
	private static final JasperReportTargetFileType DEFAULT_TARGET = JasperReportTargetFileType.XLS;
	...
	private File createNewUniquieFile(ReportDescriptor desc, String nameFmtSuffix_s) throws IOException {
		...
		final ReportContentDAO.IdRContent id = ReportContentDAO.IdRContent.createId(desc, curBasePart); //здесь имя файла = код отчёта и дата-время его создания
	...
	public byte[] generateReportTemplateByMaket(final byte[] maketData, final ReportDescriptor desc) {
		...
		try {
			...
			try {
				...
				final byte[] result =
						openOfficeExecWithRetry(new OOfficeCalcReportGeneratorImpl.Job<byte[]>() {
							...
							protected byte[] doIt() throws ConnectException {
								...
								final OpenOfficeCalcTemplateGenerator ooGen = new OpenOfficeCalcTemplateGenerator(getConnection(), getTargetDataSource());
	...
	public ReportFileData produceReport(ReportDescriptor reportDesc, ReportTemplate templateDescriptor, Map<String, Object> parameters, ReportContentDAO rptContent)
		...
		try {
			...
			final JasperReportTargetFileType target = JasperReportTargetFileType.findByName("xls", DEFAULT_TARGET);//findTargetArg(parameters);
			...
			final String msg = String.format("Fail to build openOffice calc report '%s':\n\t%s", reportDesc.getMnem(), e);
			logger.error(msg, e);
     ...
     * @param destDocFile целевой файл (например, *.xls)
     * @param target      целевой формат 
     * @throws JRException
	 */
		...
		final byte[] result =
				openOfficeExecWithRetry(new OOfficeCalcReportGeneratorImpl.Job<byte[]>() {
					...
					protected byte[] doIt() throws ConnectException {
						...
						/* Добавление атрибутов из колонок данных и сохранение в urlSave... */
						final OpenOfficeCalcFillManager filler = new OpenOfficeCalcFillManager(getConnection(), getTargetDataSource());
						try {
	 ...
	 *
	 */
	protected <TResult> TResult openOfficeExecWithRetry(OOfficeCalcReportGeneratorImpl.Job<TResult> todo) {
		int retryCount = 0;
		while (true) {
