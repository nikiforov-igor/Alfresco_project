...
(function () {
	LogicECM.module.ModelEditor.ModelControl = function (containerId, options, messages) {
		LogicECM.module.ModelEditor.ModelControl.superclass.constructor.call(this, 'LogicECM.module.ModelEditor.ModelControl', containerId);
		this.setOptions(options);
		this.setMessages(messages);
		return this;
	};

	YAHOO.extend(LogicECM.module.ModelEditor.ModelControl, Alfresco.component.Base, {
					...
					if(YAHOO.lang.isObject(model.types.type[typeIndex].properties)) {
						...
						if (p instanceof Array) {
							for ( var i = 0, n = p.length; i < n; i++) {
								...
								tmpAttributesArray[i] = {'_name':p[i]._name, 'title':p[i].title, 'type':p[i].type, 'default':p[i]['default'], 'mandatory':p[i].mandatory, '_enabled':index, 'tokenised':tokenised};//,  'validator':p[i].constraints.constraint.ref};
							}
						} else {
							...
							tmpAttributesArray.push({'_name':p._name, 'title':p.title, 'type':p.type, 'default':p['default'], 'mandatory':p.mandatory, '_enabled':index, 'tokenised':tokenised});
						}
					}
					//associations
					if(YAHOO.lang.isObject(model.types.type[typeIndex].associations)) {
						...
						if (a instanceof Array) {
							for (var i = 0, n = a.length; i < n; i++) {
								tmpAssociationsArray[i] = {'_name':a[i]._name, 'title':a[i].title,
									'class':a[i].target['class'], 'mandatory':a[i].target.mandatory, 'many':a[i].target.many};
							}
						} else {
							tmpAssociationsArray.push({'_name':a._name, 'title':a.title,
				...
				if(YAHOO.lang.isObject(model.types.type)) {
					...
					if(YAHOO.lang.isObject(model.types.type.properties)) {
						...
						if (p instanceof Array) {
							...
							var index = (YAHOO.lang.isObject(p.index)?p.index._enabled:null);
							var tokenised = (YAHOO.lang.isObject(p.index)?p.index.tokenised:null);
							tmpAttributesArray.push({'_name':p._name, 'title':p.title, 'type':p.type, 'default':p['default'], 'mandatory':p.mandatory, '_enabled':index, 'tokenised':tokenised});//,  'validator':p[i].constraints.constraint.ref});
						}
					}
					//associations
		...
		_initModelContent: function () {
			var nodeRef;
			if (this.options.itemKind === 'type' && this.options.doctype) {
				nodeRef = new Alfresco.util.NodeRef(this.options.itemId);
				Alfresco.util.Ajax.request({
					url: Alfresco.constants.PROXY_URI_RELATIVE + 'lecm/type/model?doctype=' + this.options.doctype,
					successCallback: {
						scope: this,
