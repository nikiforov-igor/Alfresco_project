...
public interface FinderBean {
	...
	/**
	 * Выполнить поисковое разименование, чтобы гарантировать наличие 
	 * аргумента argIdMain:
	 *  - если argIdMain заполнен - ничего не происходит,
	 *  - иначе, если есть argRef - выполняется поиск узла согласно search-значению 
	 *  в argRef и id найденного узла присваивается для argIdMain. 
	 * @param args
	 * @param argNodeId ключ, соот-щий основному значению, которое должно 
	 * гарантироваться поиском
	 * @param argNodeRef ключ для поискового минизапроса, которыё должен найти узел,
	 * id которого надо присвоить для argIdMain, формат ссылки такой:
