...
public class ContextMenuUI extends Composite implements SourcesPopupEvents,
				EventPreview, HasAnimation, HasCloseHandlers<ContextMenuUI>
{

	private static ContextMenuUIUiBinder uiBinder = GWT
			.create(ContextMenuUIUiBinder.class);

	interface ContextMenuUIUiBinder extends UiBinder<Widget, ContextMenuUI> {
	}
	
	@UiField FlowPanel menuwrap;
	...
	/**
	   * A callback that is used to set the position of a {@link ContextMenuUI} right
	   * before it is shown.
	   */
	  public interface PositionCallback {

	    /**
	     * Provides the opportunity to set the position of the ContextMenuUI right
	     * before the ContextMenuUI is shown. The offsetWidth and offsetHeight values
	     * of the ContextMenuUI are made available to allow for positioning based on
	     * its size.
	     * 
	     * @param offsetWidth the offsetWidth of the ContextMenuUI
	     * @param offsetHeight the offsetHeight of the ContextMenuUI
	     * @see ContextMenuUI#setPopupPositionAndShow(PositionCallback)
	     */
	  ...
	  static class ResizeAnimation extends Animation {
	    /**
	     * The {@link ContextMenuUI} being affected.
	     */
	    private ContextMenuUI curPanel = null;

	    /**
	     * The offset height and width of the current {@link ContextMenuUI}.
	     */
	     ...
	     */
	    public ResizeAnimation(ContextMenuUI panel) {
	      this.curPanel = panel;
	    }

	    /**
	     * Open or close the content. This method always called immediately after
	     * the ContextMenuUI showing state has changed, so we base the animation on the
	     * current state.
	     * 
	    ...
	    public void setState(boolean showing) {
	      ...
	      if (animate) {
	        ...
	        if (showing) {
	          ...
	          // the ContextMenuUI will appear to 'jump' from its static/relative
	  ...
	  /**
	   * The {@link ResizeAnimation} used to open and close the {@link ContextMenuUI}s.
	   */
	  ...
	  public HandlerRegistration addCloseHandler(CloseHandler<ContextMenuUI> handler) {
	   ...
	   * the ContextMenuUI or its children should be ignored.
	   * 
	   ...
	   * the next time it is shown. If the ContextMenuUI is already visible, the
	   ...
	   * ContextMenuUI or its children will be ignored.
	   * 
	   ...
	   * to actually attached/detach the {@link ContextMenuUI} to the page.
	   * 
	  ...
	  public void setVisible(boolean visible) {
	    ...
	    // to occur so the size of the ContextMenuUI can be reliably determined.
	  ...
	  protected com.google.gwt.user.client.Element getContainerElement() {
	    return menuwrap.getElement(); 
	  }

	  /**
	   * Get the glass element used by this {@link ContextMenuUI}. The element is not
	  ...
	  void maybeUpdateSize() {
	    // For subclasses of ContextMenuUI, we want the default behavior of setWidth
	    // and setHeight to change the dimensions of ContextMenuUI's child widget.
	    // We do this because ContextMenuUI's child widget is the first widget in
	    ...
	    // FlexTable, which is ContextMenuUI's child widget. However, it is not
	    // DialogBox's child widget. To make sure that we are actually getting
	    // ContextMenuUI's child widget, we have to use super.getWidget().
	  ...
	  /**
	   * Enable or disable animation of the {@link ContextMenuUI}.
	   * 
	  ...
	  private com.google.gwt.user.client.Element getPopupImplElement() {
	    return DOM.getFirstChild(getContainerElement());
	  }
