    ...
    <div id="${el}-formContainer">
       ...
       <#if result.states?? >
            <#list result.states as state>
                <div class="widget-button-grey" onclick="workflowForm.show('trans', '${nodeRef}', '${state.workflowId}', '${result.taskId}', '${state.actionId}', '', [<#list state.errors as error>'${error}'<#if error_has_next>,</#if></#list>], [<#list state.fields as field>'${field}'<#if field_has_next>,</#if></#list>], '${state.label}')">${state.label}</div>
            </#list>
       </#if>
       <#if result.workflows??>
        <#list result.workflows as workflow>
            <div class="widget-button-grey" onclick="workflowForm.show('user','${nodeRef}', '${workflow.workflowId}', '${result.taskId}', '${workflow.id}', '<#list workflow.assignees as assignee>${assignee}<#if assignee_has_next>,</#if></#list>', [<#list workflow.errors as error>'${error}'<#if error_has_next>,</#if></#list>], [<#list workflow.fields as field>'${field}'<#if field_has_next>,</#if></#list>], '${workflow.label}')">${workflow.label}</div>
        </#list>
       </#if>
    </div>
</div>
