...
public class ResourceBindPlugin implements Plugin {
	...
	{
		return new Or(new Criteria[] { ElementCriteria.name("stream:features").add(ElementCriteria.name("bind")),
				ElementCriteria.name("iq").add(ElementCriteria.name("bind", "urn:ietf:params:xml:ns:xmpp-bind")) });
	}
	...
	public boolean process(Packet iq) 
	{
		...
		} else 
		{
			if ("result".equals(iq.getAtribute("type"))) 
			{
				...
				ResourceBindPlugin.this.bindedJid = JID.fromString(jid.getCData());
				JID new_jid = JID.fromString(jid.getCData());
				user.setUsername(new_jid.getNode());
				fireOnBindResource(new_jid);
				con.getEventsManager().fireEvent(Events.resourceBinded, new ResourceBindEvenet(new IQ(iq), new_jid));
			} 
			else
			{
				stage = PluginState.ERROR;
				con.getEventsManager().fireEvent(Events.resourceBindingError, new ResourceBindEvenet(new IQ(iq), null));
			}
		}
		return true;
	}
