 ...
 *  Практически полная копия исходного адаптера, за исключением того, что
 *  в этой версии можно передавать тему и плагины (в исходной было без разницы, т.к
 *  они перезаписывались на дефолтные)
 */
Alfresco.util.RichEditorManager.addEditor('LECMTinyMCE', function(id, config)
{
   var editor;
   ...
   return (
   {
      init: function RichEditorManager_tinyMCE_init(id, config)
      {
		 ...
		 config.skin = config.skin || "lecm-editor";
         if (!config.toolbar)
         {
         ...
         config.plugins = config.plugins || [
            ...
            "table paste textcolor visualblocks"
         ];
