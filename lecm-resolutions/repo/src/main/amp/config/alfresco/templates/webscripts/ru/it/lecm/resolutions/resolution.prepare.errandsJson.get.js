...
if (resolutionRef && base.isNodeRef(resolutionRef)) {
    ...
    if (resolution) {
        ...
        if (status != "На исполнении" && status != "Аннулировано" && status != "Завершено" && status != msg.get("lecm.resolutions.statemachine-status.on-execution") && status != msg.get("lecm.resolutions.statemachine-status.annulled") && status != msg.get("lecm.resolutions.statemachine-status.completed")) {
            fromJson = true;
