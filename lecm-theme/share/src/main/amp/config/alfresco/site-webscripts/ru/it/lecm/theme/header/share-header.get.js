...
var appItems = [
	/*
	 * Достаточно злой костыль, призванный решить следующую ошибку:
	 * При процессинге виджетов учитывается то, что некоторых зависимостей может и не быть,
	 * поэтому используется механизм deferred'а с попыткой сохранить индексы элементов (которые берутся как порядок объявления)
	 * Однако, код восстановления массива выглядит следующим образом:
	 * [CoreWidgetProcessing.js]
	 ...
	 *	}
	 *
	 * Что гарантирует нарушение порядка первого элемента в том случае, если до него пришли другие
	 * 
	 * Ошибка стала проявляться после оверрайда _AlfMenuItemMixin.js (ALFFIVE-112)
	 * что как раз и увеличивает время на резолв деферреда, сдвигая первый элемент в конец 
	 */
	{
		id: "DUMMY",
