...
public class SaslAuthPlugin implements Plugin {
	...
	public SaslAuthPlugin(final Connector con, final User user, final EventsManager eventsManager) {
		this(con, user, eventsManager, null);
	}

	public SaslAuthPlugin(final Connector con, final User user, final EventsManager eventsManager, final SaslMechanism saslMechanism) {
		this.con = con;
	...
	public void auth() {
		...
		fireEventStartAuth();
		eventsManager.fireEvent(Events.saslStartAuth, new SaslEvent(mechanism, null));
	...
	public Criteria getCriteria() {
		return new Or(new Criteria[] { ElementCriteria.name("stream:features").add(ElementCriteria.name("mechanisms")),
				ElementCriteria.name("success"), ElementCriteria.name("failure") });
	}
	...
	public boolean process(Packet element) {
		if ("failure".equals(element.getName())) {
			...
			Packet x = children != null && children.size() > 0 ? children.get(0) : null;
			String msg = null;
			if (x != null)
				msg = x.getName();
			fireEventFail(msg);
			eventsManager.fireEvent(Events.saslFail, new SaslEvent(mechanism, msg));
		} else if ("success".equals(element.getName())) {
			stage = PluginState.SUCCESS;
			fireEventSuccess();
			eventsManager.fireEvent(Events.saslSuccess, new SaslEvent(mechanism, null));
