...
public class ContractStagesPolicy implements NodeServicePolicies.OnUpdatePropertiesPolicy {
    ...
    public void onUpdateProperties(NodeRef nodeRef, Map<QName, Serializable> before, Map<QName, Serializable> after) {
        ...
        if (newStatus != null && !newStatus.equals(oldStatus)) {
            if (ContractsBeanImpl.STAGE_STATUSES_CONSTR.IN_WORK.getHistoryValue().equals(newStatus) || ContractsBeanImpl.STAGE_STATUSES_CONSTR.IN_WORK.toString().equals(newStatus)) {
                nodeService.setProperty(nodeRef, ContractsBeanImpl.PROP_STAGE_START_DATE_REAL, new Date());
            } else if (ContractsBeanImpl.STAGE_STATUSES_CONSTR.CLOSED.getHistoryValue().equals(newStatus) || ContractsBeanImpl.STAGE_STATUSES_CONSTR.CLOSED.toString().equals(newStatus)) {
