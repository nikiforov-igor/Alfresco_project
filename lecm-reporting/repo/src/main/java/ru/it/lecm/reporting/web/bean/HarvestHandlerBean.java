   ...
   public void execute(ActionEvent event) {
      ...
      if(id != null && !id.equals("")) {
         ...
         if(labelId != null && !labelId.equals("")) {
            ...
            if(ReportingModel.TYPE_REPORTING_ROOT.equals(this.nodeService.getType(actionedUponNodeRef))) {
               replyString = replyString + " (ReportingRoot)";
            }

            if(ReportingModel.TYPE_REPORTING_HARVEST_DEFINITION.equals(this.nodeService.getType(actionedUponNodeRef))) {
               replyString = replyString + " (" + this.nodeService.getProperty(actionedUponNodeRef, ReportingModel.PROP_REPORTING_HARVEST_FREQUENCY) + ")";
            }

            NodeRef parent = this.nodeService.getPrimaryParent(actionedUponNodeRef).getParentRef();
            if(this.nodeService.exists(actionedUponNodeRef)) {
               Action fm = this.actionService.createAction("harvesting-executer");
               if(frequency != null) {
                  fm.setParameterValue("frequency", frequency);
               }

               try {
                  this.actionService.executeAction(fm, actionedUponNodeRef);
               } catch (UnexpectedRollbackException var16) {
                  ;
               }
            ...
            FacesMessage fm1 = new FacesMessage(FacesMessage.SEVERITY_INFO, replyString, replyString);
            context.addMessage(replyString, fm1);
         } else {
