function main() {
    ...
    var query = 'TYPE:\"lecm-contractor:contractor-type\" AND (';
    if (fullName) {
        query += '=@lecm\\-contractor\\:fullname\\-search:\"' + contractorsRootObject.formatContrractorName(fullName) + '\"';
    }
    if (shortName) {
        if (query.length > 0) {
            query += ' OR ';
        }
        query += '=@lecm\\-contractor\\:shortname\\-search:\"' + contractorsRootObject.formatContrractorName(shortName) + '\"';
    }
    if (inn && inn.length > 0) {
        if (query.length > 0) {
            query += ' OR ';
