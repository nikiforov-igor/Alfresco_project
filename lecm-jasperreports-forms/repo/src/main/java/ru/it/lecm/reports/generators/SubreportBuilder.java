...
//	final private NamespaceService nameService;
//	final private SubstitudeBean substService;
	 ...
	 * @param cfg карта соот-вий из описателя отчёта getSubItemsSourceMap();
	 */
	private void applySubItemsSourceMap()
	{
		...
		for(Map.Entry<String, String> e: cfg.entrySet() ) {
			...
			final String svalue = ((String) e.getValue()).trim();
	...
	protected Object makeSubItem( NodeRef subItemId, String subItemNum)
	{
		...
		if (beanClassIsMap()) // если требуется обычный Map/HashMap -> уже готово
			return args;
	...
	protected void assignProperties(final Object destBean
	...
	public Object buildSubreport(NodeRef docId)
	{
		...
		if (children == null || children.isEmpty()) { // нет вложенных ...
			...
			return (usingFormat) 
						? subreport.getItemsFormat().getIfEmptyTag()
						: null;
		}
	...
	protected static class AssocListInfo {
		...
		public void parseSourceLink(String svalue) {
			...
			final int iend = (svalue.endsWith(SubstitudeBean.CLOSE_SUBSTITUDE_SYMBOL))
							...
							: svalue.length();
			// svalue.indexOf(SubstitudeBean.SPLIT_TRANSITIONS_SYMBOL);
