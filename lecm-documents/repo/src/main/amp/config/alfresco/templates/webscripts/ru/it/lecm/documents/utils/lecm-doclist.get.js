...
<import resource="classpath:/alfresco/templates/webscripts/ru/it/lecm/documents/utils/lecm-docs-filters.lib.js">
...
function getDoclist() {
	...
	{
		try {
			if ((node.isContainer || node.isLinkToContainer) && !node.isSubType("lecm-document:base")) {
	...
	for each(node in nodes)
	{
		...
		if (item !== null) {
			...
			item.isLecmDocument = node.isSubType("lecm-document:base");
			items.push(item);
		}
		else {
			--totalRecords;
