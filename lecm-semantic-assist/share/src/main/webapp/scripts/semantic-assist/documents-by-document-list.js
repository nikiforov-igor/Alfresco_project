...
(function()
{
    ...
    * @return {Alfresco.SemanticSearch} The new Search instance
    * @constructor
    */
   Alfresco.SemanticSearch = function(htmlId)
   {
      Alfresco.SemanticSearch.superclass.constructor.call(this, "Alfresco.SemanticSearch", htmlId, ["button", "container", "datasource", "datatable", "paginator", "json"]);
   ...
   YAHOO.extend(Alfresco.SemanticSearch, Alfresco.component.Base,
   {
      /**
      ...
      options:
      {
		 ...
		 initialSemanticTag: "",
		 /**
		  *Noderef of document to get similar documents
		  */
		 initialSemanticNodeRef: "",

		 initialSemanticDocType: ""
      },
      ...
      onReady: function Search_onReady()
      {
         ...
         var uriSearchResults = Alfresco.constants.PROXY_URI_RELATIVE + "/lecm/br5/semantic/search?";
         ...
         YAHOO.Bubbling.fire("onSearch",
         {
			...
			semanticTag: this.options.initialSemanticTag,
			semanticNodeRef: this.options.initialSemanticNodeRef,
			semanticDocType: this.options.initialSemanticDocType
         });
      ...
      _setupDataTable: function Search_setupDataTable()
      {
          ...
          * These MUST be inline in order to have access to the Alfresco.SemanticSearch class (via the "me" variable).
          */
         var me = this;
         ...
         {
            key: "image", label: "", sortable: false, formatter: renderCellThumbnail, width: 100
         },
         {
            key: "summary", label: "", sortable: false, formatter: renderCellDescription
         }];
      ...
      onSearch: function Search_onSearch(layer, args)
      {
         ...
         if (obj !== null)
         {
			var semanticDocType = this.semanticDocType;
            if (obj.semanticDocType !== undefined)
            {
               semanticDocType = obj.semanticDocType;
            }
			var semanticNodeRef = this.semanticNodeRef;
            if (obj.semanticNodeRef !== undefined)
            {
               semanticNodeRef = obj.semanticNodeRef;
            }
			var semanticTag = this.semanticTag;
            if (obj.semanticTag !== undefined)
            {
            ...
            this._performSearch(
            {
			   ...
			   semanticDocType: semanticDocType
            });
         }
      },
      ...
      _performSearch: function Search__performSearch(args)
      {
         var searchTerm = YAHOO.lang.trim(args.searchTerm),
             ...
             searchRepository = args.searchRepository,
             searchSort = args.searchSort,
			 semanticTag = args.semanticTag,
			 semanticNodeRef = args.semanticNodeRef,
			 semanticDocType = args.semanticDocType;
         ...
         function successHandler(sRequest, oResponse, oPayload)
         {
			...
			this.semanticNodeRef = semanticNodeRef;
			this.semanticDocType = semanticDocType;
         ...
         this.widgets.dataSource.sendRequest(this._buildSearchParams(searchRepository, searchAllSites, searchTerm, searchTag, searchSort, semanticTag,semanticNodeRef,semanticDocType),
         {
       ...
       */
      _buildSearchParams: function Search__buildSearchParams(searchRepository, searchAllSites, searchTerm, searchTag, searchSort, semanticTag, semanticNodeRef,semanticDocType)
      {
         var site = searchAllSites ? "" : this.options.siteId;
         var params = YAHOO.lang.substitute("site={site}&term={term}&tag={tag}&maxResults={maxResults}&sort={sort}&query={query}&repo={repo}&semanticTag={semanticTag}&semanticNodeRef={semanticNodeRef}&semanticDocType={semanticDocType}",
         {
			...
			semanticTag: encodeURIComponent(semanticTag),
			semanticNodeRef: encodeURIComponent(semanticNodeRef),
			semanticDocType: encodeURIComponent(semanticDocType)
         });
      ...
      _setDefaultDataTableErrors: function Search__setDefaultDataTableErrors(dataTable)
      {
         ...
         dataTable.set("MSG_EMPTY", msg("message.empty", "Alfresco.SemanticSearch"));
         dataTable.set("MSG_ERROR", msg("message.error", "Alfresco.SemanticSearch"));
      }
   });
})();