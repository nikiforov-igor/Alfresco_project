      ...
      try
      {
         ...
         if (itemNode !== null)
         {
            ...
            for (var i = 0, ii = propNames.length; i < ii; i++)
            {
               ...
               {
                  continue;
               } else if (propName === "cm:name")
               {
                   var tempCmName = itemNode.properties[propName] + " copy";
                   if (itemNode.getParent().childByNamePath(tempCmName) !== null) {
                        var i = 1;
                        while (itemNode.getParent().childByNamePath(tempCmName + i) !== null) {
                            i++;
                        }
                        tempCmName = tempCmName + i;
                   }
                   duplicateProperties[propName] = tempCmName;
               } else {
