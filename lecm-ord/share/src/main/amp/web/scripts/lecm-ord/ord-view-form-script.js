                ...
                successCallback: {
                    fn: function (response) {
                        if (response && response.json.formatString) {
                            ...
                            var statusOk = (status == "На исполнении")
                                || (status == "Работа завершена")
                                || (status == Alfresco.util.message("lecm.ord.statemachine-status.on-execution"))
                                || (status == Alfresco.util.message("lecm.ord.statemachine-status.work-completed"));
                            if (!statusOk) {
