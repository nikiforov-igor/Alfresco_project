...
(function() {
    function drawForm(){
        var control = new LogicECM.module.Review.DocumentTable('${controlId}', '${itemId}').setMessages(${messages});
        control.setOptions(
                {
                    reportId: '${reportId}',
    ...
    function init() {
        LogicECM.module.Base.Util.loadResources([
                    ...
                    'scripts/lecm-review/review-document-table.js'
                ],
                [
...
<div id="${controlId}">
    ...
    <div id="${controlId}-review-container" class="reviewContainer">
        <div class="reviewToolbarContainer">
            <div class="reviewControlsContainer">
            <#if editable><a id="${controlId}-rejectReview" class="rejectReview" href="javascript:void(0);" title="${msg('title.reject')}"></a></#if>
                <a id="${controlId}-printReviewReport" class="printReviewReport" href="javascript:void(0);" title="${msg('title.print')}"></a>
            </div>
        </div>
        <div id="${containerId}">
