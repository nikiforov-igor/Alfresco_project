...
<div class="dashlet arm-documents">
    <div class="title">СЭД: <#if settings?? && (settings.title?length == 0 || settings.dashletTitle?length == 0)>Дашлет не настроен<#else>${settings.title}</#if></div>
    ...
    <div id="${id}-main" class="body scrollableList" <#if args.height??>style="height: ${args.height}px;"</#if>>
        ...
        <div id="${id}-documents-loading" style="text-align: center; visibility: hidden;">
