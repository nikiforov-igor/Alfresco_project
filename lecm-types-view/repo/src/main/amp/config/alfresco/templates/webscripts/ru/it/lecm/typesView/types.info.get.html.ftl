		...
		<#list types["meta"] as typeMeta>
			<tr>
				<td colspan="6" align="center">
					<a class="atype" href="${url.context}/service/lecm/typesView/info?typeName=${typeMeta['name']}" target="_blank">
						${typeMeta['name']}
						<#if typeMeta['desc']??>
							(${typeMeta['desc']!""})
						</#if>
	...
	<#list orders?sort as order>
		...
		<#list assocs as assoc>
			<tr>
				...
				<td>
					<#if assoc["target"]??>
						<a class="assoctype" href="${url.context}/service/lecm/typesView/info?typeName=${assoc['target']}" target="_blank">
							${assoc['target']}
							<#if assoc['targetDesc']??>
								(${assoc['targetDesc']!""})
							</#if>
...
<table>
	...
	<#list orders?sort as order>
		...
		<#list aspects as aspect>
			<tr>
				<td>
					<a class="assoctype" href="${url.context}/service/lecm/typesView/info?typeName=${aspect['name']}" target="_blank">
						${aspect["name"]}
					</a>
				</td>
				<td>${aspect["desc"]!""}</td>
