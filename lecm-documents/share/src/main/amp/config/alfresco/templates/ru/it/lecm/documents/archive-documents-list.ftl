    ...
    LogicECM.currentUser = <#if currentUser?? >'${currentUser}'<#else>null</#if>;
        ...
        <#if filter == "">
                var PREFERENCE_DOCUMENTS_STATUSES =
                        ARCHIVE_DOCS + (("${docType}" != "") ? "${docType}" : "lecm-base:document").split(":").join("_") + ".documents-list-statuses-filter"  + ".${currentUser}";
        ...
        <#if queryFilterId?? && queryFilterId != "">
            var PREFERENCE_FILTER = ARCHIVE_DOCS
                    + (("${docType}" != "") ? "${docType}" : "lecm-base:document").split(":").join("_") + "." + "${queryFilterId}" + ".${currentUser}";
