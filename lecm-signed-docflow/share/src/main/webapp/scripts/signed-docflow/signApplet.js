...
var cryptoAppletModule = (function () {
	var certContainer = 'container',
		CURRENT_CONTAINER = '',
		attributes = { id: 'signApplet', width: 1, height: 1 },
		parameters = {jnlp_href: 'http://127.0.0.1:8080/share/scripts/ItStampApplet.jnlp'},
		config = {
			"tsPolicy": null,
			"proxyPass": null,
			"proxyUser": null,
	...
	function getCertInfo(container) {
		var result = { container: container },
			Info = JSON.parse(signApplet.getService().certInfo(signApplet.getService().bytesToBase64(signApplet.getService().getCertFromStore('', container)))),
		...
		sd.setOptions({
			...
			doBeforeAjaxRequest : {
				fn: function(form) {
					var i,
						nodeRefList = [],
						fields = document.forms["multiple-sign-form"].getElementsByTagName("input");
	...
	var multipleSign = function(form) {
		var i,
			...
			signObj,
			certInfo,
			container = cryptoAppletModule.getCurrentContainer(),
			sd = new Alfresco.module.SimpleDialog("dialog");
		sd.setOptions({
			width: "20em",
			templateUrl: "components/form",
			destroyOnHide: true,
		...
		for(i = 0; i < fields.length; i++) {
			...
			attachSign = signApplet.sign(Alfresco.constants.PROXY_URI + "api/node/content/" + contentURI, "URL");
			signObj = {
				"sign-to-content-association" : nodeRef,
				"content" : attachSign,
				"signing-date" : signDate
				};
			certInfo = getCertInfo(container);
