...
public class ChatManager<T> {
	...
	private void fireSyncSend(Chat<T> chat, Message message,boolean firstMessage) {
		for (ChatListener<T> l : this.chatListeners) {
			l.onSyncSend(chat, message,firstMessage);
		}
	}
	
	private void fireSyncRecv(Chat<T> chat, Message message,boolean firstMessage) {
		for (ChatListener<T> l : this.chatListeners) {
	...
	void process(Message message) 
	{
		...
		Chat<T> chat = getChat(fromJid, threadId);
		boolean firstMessage = false;
		if(chat==null&&(message.getBody()==null||message.getBody().length()==0))
			return;
		if (chat == null) 
		{
			chat = new Chat<T>(this, fromJid, threadId);
