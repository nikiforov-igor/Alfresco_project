...
(function () {
    ...
    YAHOO.lang.augmentObject(LogicECM.ORD.PointsTS.prototype, {
                    ...
                    fn: function (response) {
                        if (response && response.json.user) {
                            ...
                            if (allowedStatuses.indexOf(docStatus) >= 0 &&
                                ((docStatus == this.msg("lecm.ord.statemachine-status.on-registration") && currentUser.roles.indexOf("DA_REGISTRAR_DYN") >= 0 && currentUser.roles.indexOf("DA_REGISTRARS") >= 0) ||
                                (docStatus != this.msg("lecm.ord.statemachine-status.on-registration") && currentUser.roles.indexOf("BR_INITIATOR") >= 0))) {
        ...
        showCompleteActionEvaluator: function (rowData) {
            ...
            var isStatusOk = itemStatus && itemStatus.displayValue == Alfresco.util.message("lecm.ord.item.status.on-execution");
        ...
        showActionsEvaluator: function (rowData) {
            var itemStatus = rowData.itemData["assoc_lecm-ord-table-structure_item-status-assoc"];
            var isItemStatusOk = itemStatus && itemStatus.displayValue == Alfresco.util.message("lecm.ord.item.status.wait-for-execution");
            var isEmployeeOk =
                ((this.options.currentDocStatus == Alfresco.util.message("lecm.ord.statemachine-status.on-registration") && this.options.currentUser.roles.indexOf("DA_REGISTRAR_DYN") >= 0 && this.options.currentUser.roles.indexOf("DA_REGISTRARS") >= 0) ||
                (this.options.currentDocStatus != Alfresco.util.message("lecm.ord.statemachine-status.on-registration") && this.options.currentUser.roles.indexOf("BR_INITIATOR") >= 0));
