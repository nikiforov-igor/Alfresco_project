...
(function() {
	...
	function Absence_DrawInstantElement() {
		...
		if (!Alfresco.module.SimpleDialog || LogicECM.module.WCalendar.Absence.isAbsent) {
			parentID = htmlNode.parentNode.id;
			parentNode = new YAHOO.util.Element(parentID);
			parentNode.setStyle("display", "none");
		} else if (Alfresco.module.SimpleDialog && !LogicECM.module.WCalendar.Absence.isAbsent) {
			parentID = htmlNode.parentNode.id;
			parentNode = new YAHOO.util.Element(parentID);
