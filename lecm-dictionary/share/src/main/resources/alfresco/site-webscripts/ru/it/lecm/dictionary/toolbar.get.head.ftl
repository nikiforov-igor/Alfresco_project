<#--<#include "component.head.inc">-->
<!-- Data List Toolbar -->
<@link rel="stylesheet" type="text/css" href="${page.url.context}/res/components/data-lists/toolbar.css" />
<#--<@script type="text/javascript" src="${page.url.context}/res/components/data-lists/toolbar.js"></@script>-->
<script type="text/javascript" src="${page.url.context}/res/ru/it/lecm/dictionary/dictionary_toolbar.js"></script>
<#--<script type="text/javascript" src="${url.context}/yui/other/toolbar.js"></script>-->