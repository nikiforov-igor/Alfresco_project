...
public class ReportingHelper {
    ...
    private DataSource alfrescoDataSource;
    ...
    public DataSource getAlfrescoDataSource() {
        return alfrescoDataSource;
    }

    public void setAlfrescoDataSource(DataSource alfrescoDataSource) {
        this.alfrescoDataSource = alfrescoDataSource;
    }
    ...
    public void initializeReportingRoot(ReportingRoot reportingRoot) {
        reportingRoot.setGlobalExecutionEnabled((Boolean) this.nodeService.getProperty(reportingRoot.getNodeRef(), ReportingModel.PROP_REPORTING_GLOBAL_EXECUTION_ENABLED));
        reportingRoot.setHarvestEnabled((Boolean) this.nodeService.getProperty(reportingRoot.getNodeRef(), ReportingModel.PROP_REPORTING_HARVEST_ENABLED));
    ...
    public void initializeReportingContainer(ReportingContainer reportingContainer) {
        reportingContainer.setExecutionEnabled((Boolean) this.nodeService.getProperty(reportingContainer.getNodeRef(), ReportingModel.PROP_REPORTING_EXECUTION_ENABLED));
    ...
    public Map<String, String> getNameSpaces() {
        if(this.namespaces == null) {
            ...
            for (String key : keys) {
                String namespaceURI = this.serviceRegistry.getNamespaceService().getNamespaceURI(key);
                String into = key.replace("-", "_") + "_";
                this.namespaces.put(namespaceURI, into);
                if (logger.isDebugEnabled()) {
                    logger.debug("getNameSpaces: Replacing: " + namespaceURI + " into: " + into);
                }
            }
        }
        ...
        if (configsRoot != null) {
            ...
            for (ChildAssociationRef child : childs) {
                ...
                if (configName.toLowerCase().endsWith(".properties")) {
                    ...
                    } catch (ContentIOException | IOException var5) {
